class UserRegistration:
    def __init__(self):
        ...
        Each entry in the dictionary will map an email to a dictionary containing the user's 
    ...
    def register(self, email, password, confirm_password):
        ...
        This function takes an email, password, and password confirmation as input.
        It performs a series of checks to ensure the registration 
        is valid:
        ...
        If all checks pass, the user is registered, and their email and password are stored 
        in the `users` dictionary, along with a confirmation status set to False 
        (indicating the user is not yet confirmed). A success message is returned.
        ...
        Returns:
            ...
            On success, it returns {"success": True, "message": 
    ...
    def is_valid_email(self, email):
        ...
        This rule only checks that the email contains
        an '@' symbol and has a '.' in the domain part.
...
class TestUserRegistration(unittest.TestCase):
    ...
    def test_successful_registration(self):
        ...
        It verifies that a valid email and matching strong
        password results in successful registration.
        """
        result = self.registration.register("user@example.com", "Password123", "Password123")
        # Ensures that registration is successful.
    ...
    def test_invalid_email(self):
        ...
        It verifies that attempting to register with 
        an incorrectly formatted email results in an error.
        """
        result = self.registration.register("userexample.com", "Password123", "Password123")

        # Ensures registration fails due to invalid email.
        self.assertFalse(result['success'])

        # Checks the specific error message.
        self.assertEqual(result['error'], "Invalid email format")

    def test_password_mismatch(self):
        """
        Test case for password mismatch.
        It verifies that when the password and confirmation
        password do not match, registration fails.
        """
        result = self.registration.register("user@example.com", "Password123", "Password321")

        # Ensures registration fails due to password mismatch.
        self.assertFalse(result['success'])
    ...
    def test_weak_password(self):
        ...
        # Ensures registration fails due to a weak password.
        self.assertFalse(result['success'])
    ...
    def test_email_already_registered(self):
        ...
        It verifies that attempting to register an email that has 
        already been registered results in an error.
        """
        ...
        # Ensures registration fails due to the email already being registered.
        self.assertFalse(result['success'])
